...
import {reducer, reset, sendMessageToFeed, setGrouping} from "../components/orderbookFns";

const useOrderBook = (url: string, product_ids: string[]) => {
    const initialState = <IState>{asks: [], bids: [], currentGrouping: TicketSize.BTCDefault};
    const [state, dispatch] = useReducer(reducer, initialState);

    const {sendJsonMessage, lastJsonMessage, readyState} = useWebSocket(url, {
        onError: ()=>alert("websocket failed")
    });
    const sendMessageEvent = curry(sendMessageToFeed)(sendJsonMessage, "book_ui_1");
