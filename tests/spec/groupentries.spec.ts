...
describe("Group stream by ticketsize", () => {
  it("Groups bitcoin by 2.5", () => {
    ...
    let sum1 =
      Math.floor(
        (orderbook[0][0] + orderbook[1][0] + orderbook[2][0]) /
          3 /
          TicketSize.BTCLarge
      ) * TicketSize.BTCLarge;
    expect(newOrderbook[0][0]).to.equal(orderbook[2][0]);
    expect(newOrderbook[0][1]).to.equal(
      orderbook[0][1] + orderbook[1][1] + orderbook[2][1]
    );

    let sum2 =
      Math.floor(
        (orderbook[3][0] + orderbook[4][0] + orderbook[5][0]) /
          3 /
          TicketSize.BTCLarge
      ) * TicketSize.BTCLarge;
    expect(newOrderbook[1][0]).to.equal(orderbook[5][0]);
    expect(newOrderbook[1][1]).to.equal(
      orderbook[3][1] + orderbook[4][1] + orderbook[5][1]
    );

    let sum3 =
      Math.floor(orderbook[6][0] / 1 / TicketSize.BTCLarge) *
      TicketSize.BTCLarge;
  ...
  it("Groups bitcoin by ticketsize 1", () => {
    ...
    let sum1 =
      Math.floor(
        (orderbook[0][0] + orderbook[1][0]) / 2 / TicketSize.BTCMedium
      ) * TicketSize.BTCMedium;
    expect(newOrderbook[0][0]).to.equal(orderbook[1][0]);
    expect(newOrderbook[0][1]).to.equal(orderbook[0][1] + orderbook[1][1]);

    let sum2 =
      Math.floor(
        (orderbook[2][0] + orderbook[3][0]) / 2 / TicketSize.BTCMedium
      ) * TicketSize.BTCMedium;
